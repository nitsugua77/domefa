...
class SessionProService
{
    ...
    public function __construct(Medecins $users, EntityManager $em)
    {
        ...
        /*else if ($verif === 0 && $this->checkMedecin($_SESSION['REMOTE_USER']))
        {
            $this->users = $users;
    ...
    /*public function checkMedecin($rpps)
    {
        try {
            if ($this->validSession === FALSE) {
            ...
            if (isset($rpps)) {
                $results = $this->getEm()->getRepository('Application\Models\Entity\Medecins')->findOneBy(array('rpps' => $rpps));
            }
            $this->validSession = $this->session_obliterate();
